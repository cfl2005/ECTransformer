...
if __name__ == '__main__':
    ...
    p1 = mp.Process(target=translate_texts, args=(split_sents[0], model))
    p2 = mp.Process(target=translate_texts, args=(split_sents[1], model))
    p3 = mp.Process(target=translate_texts, args=(split_sents[0], model))
    print('Start process...')
    start = time.time()
    p1.start()
    p2.start()
