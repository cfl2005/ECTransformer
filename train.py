...
import config
from beam_decoder import beam_search, beam_search_decode
...
def translate_decode(src_enc, src_mask, model, use_beam=True):
    ...
    with torch.no_grad():
        if use_beam:
            decode_result, _ = beam_search(model, src_enc, src_mask, config.max_len,
                                           config.padding_idx, config.bos_idx, config.eos_idx,
                                           config.beam_size, config.device, encode=0)
            decode_result = [h[0] for h in decode_result]
        else:
            decode_result = batch_greedy_decode(model, src_enc, src_mask, 
                                                max_len=config.max_len, encode=0)
        translation = [sp_chn.decode_ids(_s) for _s in decode_result]
        return translation
...
def translate_decode_split(src, src_mask, model_decode, model_generator, use_beam=True):
    ...
    with torch.no_grad():
        if use_beam:
            decode_result, _ = beam_search_decode(model_decode, model_generator, 
                                            src, src_mask, config.max_len,
                                           config.padding_idx, config.bos_idx, config.eos_idx,
                                           config.beam_size, config.device)
            decode_result = [h[0] for h in decode_result]
        else:
