...
def batch_greedy_decode(model, src, src_mask, max_len=64, start_symbol=2, end_symbol=3, encode=1):
...
class Transformer_decode(nn.Module):
    def __init__(self, decoder, tgt_embed):
        super(Transformer_decode, self).__init__()
        self.decoder = decoder
        self.tgt_embed = tgt_embed
...
# 创建拆分模型
def make_split_model(src_vocab, tgt_vocab, N=6, d_model=512, d_ff=2048, h=8, dropout=0.1):
    ...
    return model_encode.to(DEVICE), model_decode.to(DEVICE), generator.to(DEVICE)

# 创建encode模型
def make_model_encode(src_vocab, tgt_vocab, N=6, d_model=512, d_ff=2048, h=8, dropout=0.1):
    # gpu_tracker = MemTracker() 
    # gpu_tracker.track()                  # 开始检测    
    ...
    return model_encode.to(DEVICE)

# 创建模型: decode, generator
def make_model_decode(src_vocab, tgt_vocab, N=6, d_model=512, d_ff=2048, h=8, dropout=0.1):
    # gpu_tracker = MemTracker() 
    # gpu_tracker.track()                  # 开始检测    
    ...
    return model_decode.to(DEVICE), generator.to(DEVICE)
